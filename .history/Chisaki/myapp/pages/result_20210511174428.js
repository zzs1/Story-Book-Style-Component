...
export default function Result() {
  ...
  const [severe, setLevel1]= useState(null);
  const [mild, setLevel2]= useState(null);
  const [moderate, setLevel3]= useState(null);
  ...
  var Levels = SliderUI.level

        if (A,B,C<= 51)
        {
          setLevel1Levels="Severe"
        }
        else if (A,B,C >= 25)
        {
          setLevel2(mild)
